...
    if((semid = creatsem("./", 39, 2, 0)) == -1)return -1;
    // sem idx: 0 - init 1 (writable), 1 - init 0 (nothing to read)
    union semun init_val;
    init_val.val = 1;
    if(semctl(semid, 0, SETVAL, init_val) == -1)perror("semctl set 0 error");
    init_val.val = 0;
    if(semctl(semid, 1, SETVAL, init_val) == -1)perror("semctl set 1 error");
    printf("Server (write) Started\n");
    printf("Enter text to send ('#' to quit):\n");
    while(1){
        sem_p(semid, 0);    // P op: apply write
        printf("write: ");
        if(fgets(write_str, SHM_SIZE, stdin) == NULL)break;
